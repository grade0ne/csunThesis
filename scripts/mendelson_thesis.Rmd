...
fig1a_data <- exp1_growth_summary %>%
  ...
  summarize(mean_mu = mean(mumax), ci_mu = sd(mumax)/sqrt(length(mumax)))
...
fig1a <- ggplot(fig1a_data, aes(x = Treatment, y = mean_mu, color = Leaf, group = Clone)) +
            ...
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean_mu - ci_mu, ymax = mean_mu + ci_mu), width = 0.15) +
...
fig1b_data <- exp1_bodysize %>%
  ...
  summarize(mean_size = mean(area_px), ci_size = sd(area_px)/sqrt(length(area_px)))

fig1b <- ggplot(fig2_data, aes(x = temperature, y = mean_size, color = leaf, group = clone)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean_size - ci_size, ymax = mean_size + ci_size), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
            facet_wrap(~leaf) +
...
fig2a_data <- exp2_growth_summary_rotifer %>%
  ...
  summarize(mean = mean(r_day), ci = sd(r_day)/sqrt(length(r_day)))
...
fig2b_data <- exp2_growth_summary_rotifer %>%
  group_by(evolvedTemp, currentTemp, competition) %>%
  summarize(mean = mean(log(r_day)), ci = sd(log(r_day))/sqrt(length(r_day)))
