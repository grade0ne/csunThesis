...
fig1a_data <- exp1_growth_summary %>%
  ...
  summarize(mean_mu = mean(mumax), ci_mu = 1.96 * sd(mumax)/sqrt(length(mumax)))
...
fig1a <- ggplot(fig1a_data, aes(x = Treatment, y = mean_mu, color = Leaf, group = Clone)) +
            ...
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean_mu - ci_mu, ymax = mean_mu + ci_mu), width = 0.15, linewidth = 0.25) +
...
ggsave(
  ...
  height = 4,
  units = "in",
  path = "C:/Users/timet/Documents/projectCSUN/csunThesisRepo/output/manuscriptFigures"
)


fig1b_data <- exp1_bodysize %>%
  group_by(temperature, site, leaf, clone) %>%
  summarize(mean_size = mean(area_px), ci_size = 1.96 * sd(area_px)/sqrt(length(area_px)))

fig1b <- ggplot(fig1b_data, aes(x = temperature, y = mean_size, color = leaf, group = clone)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean_size - ci_size, ymax = mean_size + ci_size), width = 0.15, linewidth = 0.25) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
            facet_wrap(~leaf) +
...
fig2a_data <- exp2_growth_summary_rotifer %>%
  group_by(evolvedTemp, currentTemp, competition) %>%
  summarize(mean = mean(r_day), ci = 1.96 * sd(r_day)/sqrt(length(r_day)))

fig2a <- ggplot(fig2a_data, aes(x = competition, y = mean, color = evolvedTemp, group = evolvedTemp)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean - ci, ymax = mean + ci), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
            facet_wrap(~currentTemp) +
            labs(x = "Competition", y = "Intrinsic growth rate") +
            theme_minimal() +
...
fig2b_data <- exp2_growth_summary_rotifer %>%
  group_by(evolvedTemp, currentTemp, competition) %>%
  summarize(mean = mean(log(r_day)), ci = 1.96 * sd(log(r_day))/sqrt(length(r_day)))

fig2b <- ggplot(fig2b_data, aes(x = competition, y = mean, color = evolvedTemp, group = evolvedTemp)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean - ci, ymax = mean + ci), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
...
fig2c_data <- exp2_growth_summary_rotifer %>%
  group_by(evolvedTemp, currentTemp, competition) %>%
  summarize(mean = mean(alpha_day), ci = 1.96 * sd(alpha_day)/sqrt(length(alpha_day)))

fig2c <- ggplot(fig2c_data, aes(x = competition, y = mean, color = evolvedTemp, group = evolvedTemp)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean - ci, ymax = mean + ci), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
...
fig2d_data <- exp2_growth_summary_protist %>%
  group_by(compFactor, currentTemp) %>%
  summarize(mean = mean(log(r_day)), ci = 1.96 * sd(log(r_day))/sqrt(length(r_day)))

fig2d <- ggplot(fig2d_data, aes(x = currentTemp, y = mean, color = compFactor, group = compFactor)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean - ci, ymax = mean + ci), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
            facet_wrap(~compFactor) +
...
fig2e_data <- exp2_growth_summary_protist %>%
  group_by(compFactor, currentTemp) %>%
  summarize(mean = mean(log(alpha_day)), ci = 1.96 * sd(log(alpha_day))/sqrt(length(alpha_day)))

fig2e <- ggplot(fig2e_data, aes(x = currentTemp, y = mean, color = compFactor, group = compFactor)) +
            geom_point(stat = "identity", position = position_dodge(width = 0.1)) +
            geom_errorbar(stat = "identity", position = position_dodge(width = 0.1),
                          aes(ymin = mean - ci, ymax = mean + ci), width = 0.15) +
            geom_line(stat = "identity", position = position_dodge(width = 0.1)) +
